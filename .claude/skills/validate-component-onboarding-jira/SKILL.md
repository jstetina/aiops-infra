...
RHOAI tickets must include `architectures`, `target_rhoai_version` (canonical form, e.g. `3.4` or `3.4-ea-2`), and `release_category` (`Generally Available` or `Tech Preview`) in addition to the common required fields. ODH tickets must include `build_type`. When `build_type` is `Release`, ODH tickets must also include `odh_release_tag` (the version tag for the release, e.g. `2.21.0`).
